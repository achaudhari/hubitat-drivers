...
metadata {
    definition (
        ...
        command "publish", [[name: "topic", type: "STRING"], [name: "payload", type: "STRING"]]
    }
}
...
def publish(String topic, String payload) {
    ...
    if (!interfaces.mqtt.isConnected()) {
        log.error "Frigate MQTT Bridge: Cannot publish - not connected to MQTT"
        return
    }

    try {
        if (debugLogging) {
            log.debug "Frigate MQTT Bridge: Publishing to topic: ${topic} with payload: ${payload}"
        }

        interfaces.mqtt.publish(topic, payload, 1, false)
        log.info "Frigate MQTT Bridge: Published to ${topic}: ${payload}"

    } catch (Exception e) {
        log.error "Frigate MQTT Bridge: Error publishing to ${topic}: ${e.message}"
    }
}
